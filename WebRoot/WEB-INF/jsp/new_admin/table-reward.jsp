									...
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											<td><input type="checkbox" name="post[]"
												value="${item.rewardId}"></td>
											<td>${item.rewardId}</td>
											<td><a href="${item.users.headSculpture}" /><img
												alt="${item.users.name}" src="${item.users.headSculpture}"
												class="col-xs-12"></a></td>
											<td>${item.users.name}</td>
											<td>${item.count}</td>
											<td><c:choose>
													<c:when test="${ item.users.platform==0}">
											Androoid设备
											</c:when>
											...
											<td><a
												...
												data-href="deleteUser.action?contentId=${item.rewardId }&menu=1&sortmenu=1&submenu=1"
...
<script type="text/javascript">
	$("a[name='page']").click(
			function() {
				...
				var submenu = ${submenu};
				var url = "rewardList.action?" + "size=" + size + "&page="
