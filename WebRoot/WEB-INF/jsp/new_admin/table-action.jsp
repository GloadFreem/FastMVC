									...
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											<td><input type="checkbox" name="post[]"
												value="${item.actionId}"></td>
											<td>${item.actionId}</td>
											<td><a href="${item.startPageImage}" target="blank" /><img
												alt="${item.name}" src="${item.startPageImage}"
												class="col-xs-12"></a></td>
											<td>${item.name}</td>
											<td><c:choose>
													<c:when test="${item.description.length()>30}">
...
<script type="text/javascript">
	$("a[name='page']").click(
			function() {
				...
				var submenu = ${submenu};
				var url = "actionList.action?" + "size=" + size + "&page="
