									...
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											<td><input type="checkbox" name="post[]"
												value="${item.sceneId}"></td>
											<td>${item.sceneId}</td>
											<td>${item.project.fullName}</td>
											<td><a href="${item.audioPath}" target="blank">查看音频</a></td>
											<td>${item.totlalTime}</td>
											<td>${item.beginTime}</td>
											<td>${item.endTime}</td>
											<td><a
												href="sceneDetail.action?contentId=${item.sceneId }&menu=4&sortmenu=3&submenu=1"
												class="active"><i
													class="fa fa-edit text-success text-active"></i><i
													class="fa fa-edit text-danger text"></i></a> | <a href="#modal"
												data-href="deleteUser.action?contentId=${item.sceneId }&menu=1&sortmenu=1&submenu=1"
...
<script type="text/javascript">
	$("a[name='page']").click(
			function() {
				...
				var submenu = ${submenu};
				var url = "sceneList.action?" + "size=" + size + "&page="
