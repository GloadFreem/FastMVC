									...
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											<td><input type="checkbox" name="post[]"
												value="${item.projectId}"></td>
											<td>${item.projectId}</td>
											<td><a href="${item.startPageImage}" target="blank" /><img
												alt="${item.abbrevName}" src="${item.startPageImage}"
												class="col-xs-12"></a></td>
											<td>${item.fullName}</td>
											<td>${item.industoryType}</td>
											<td>${item.address}</td>
								<%-- 			<td><c:choose>
													<c:when test="${ item.platform==0}">
											Androoid设备
											</c:when>
													<c:otherwise>iOS设备</c:otherwise>
												</c:choose></td> --%>
											<td>${item.financestatus.name}</td>
											<td><a
												href="projectDetail.action?contentId=${item.projectId }&menu=4&sortmenu=1&submenu=1"
												class="active"><i
													class="fa fa-edit text-success text-active"></i><i
													class="fa fa-edit text-danger text"></i></a> | <a href="#modal"
												data-href="deleteUser.action?contentId=${item.projectId }&menu=4&sortmenu=1&submenu=1"
...
<script type="text/javascript">
	$("a[name='page']").click(
			function() {
				...
				var submenu = ${submenu};
				var url = "projectList.action?" + "size=" + size + "&page="
