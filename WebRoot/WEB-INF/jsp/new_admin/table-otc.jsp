								...
								<c:when test="${result != null}">
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											...
											<td>${item.cid}</td>
											<td>${item.CCode}</td>
											<td>${item.CFullname}</td>
											<td>${item.CCorporate}</td>
											<td>${item.sharesType}</td>
											<td>${item.sharesNum}</td>
											<td>${item.securitiesTrader}</td>
											<td>${item.CType}</td>
											<td>${item.CAddress}</td>
											<td><a
												href="userDetail.action?contentId=${item.cid }&menu=1&sortmenu=1&submenu=1"
												class="active"><i
													class="fa fa-edit text-success text-active"></i>
												<iclass ="fa fa-edit text-dangertext">
													</i></a> | <a href="#modal"
												data-href="deleteUser.action?contentId=${item.cid }&menu=1&sortmenu=1&submenu=1"
												data-toggle="modal" class="active"><i
													class="fa fa-trash-o text-success text-active"></i><i
													class="fa fa-trash-o text-danger text"></i></a></td>
										</tr>
									</c:forEach>
								</c:when>
							</c:choose>
...
<script type="text/javascript">
	$("a[name='page']").click(
			function() {
				...
				var submenu = ${submenu};
				var url = "newOTCList.action?" + "size=" + size + "&page="
