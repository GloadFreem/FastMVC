									...
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											<td><input type="checkbox" name="post[]"
												value="${item.vid}"></td>
											<td>${item.vid}</td>
											<td><a href = "${item.vimage}" target="blank"/><img alt="${item.vname}" src="${item.vimage}" class="col-xs-12"></a></td>
											<td>${item.vname}</td>
											<td>${item.businessSchool.bname}</td>
											<td>${item.vdesc}</td>
											<td>${item.vposition}</td>
											<td><a href="${item.vurl}" target="blank">${item.vurl}</a></td>
											<td>${item.vtimelong}</td>
											<td><a href="sourceDetail.action?contentId=${item.vid }&menu=1&sortmenu=1&submenu=3" class="active"><i
													class="fa fa-edit text-success text-active"></i><i
													class="fa fa-edit text-danger text"></i></a> | <a
												href="#modal" data-href="deleteSource.action?contentId=${item.vid }&menu=1&sortmenu=1&submenu=3" data-toggle="modal"
												class="active"><i
