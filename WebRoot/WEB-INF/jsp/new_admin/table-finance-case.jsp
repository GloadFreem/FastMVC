									...
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											<td><input type="checkbox" name="post[]"
												value="${item.financingCaseId}"></td>
											<td>${item.financingCaseId}</td>
											<td>${item.project.fullName}</td>
											<td>${item.url}</td>
											<td>${item.content}</td>
											<td><a href="${item.url}" target="blank">查看内容</a></td>
											<td><a
												href="financeCaseDetail.action?contentId=${item.financingCaseId }&menu=4&sortmenu=6&submenu=2"
												class="active"><i
													class="fa fa-edit text-success text-active"></i><i
													class="fa fa-edit text-danger text"></i></a> | <a href="#modal"
												data-href="deleteUser.action?contentId=${item.financingCaseId }&menu=1&sortmenu=1&submenu=1"
...
<script type="text/javascript">
	$("a[name='page']").click(
			function() {
				...
				var submenu = ${submenu};
				var url = "financeCaseList.action?" + "size=" + size + "&page="
