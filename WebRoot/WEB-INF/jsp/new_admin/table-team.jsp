									...
									<c:forEach items="${result}" var="item" varStatus="status">
										<tr>
											<td><input type="checkbox" name="post[]"
												value="${item.personId}"></td>
											<td>${item.personId}</td>
											<td><a href="${item.icon}" /><img
												alt="${item.name}" src="${item.icon}"
											...
											<td>${item.url}</td>
											<td><a
												href="teamDetail.action?contentId=${item.personId }&menu=4&sortmenu=9&submenu=2&requestType=webRequest"
												class="active"><i
													class="fa fa-edit text-success text-active"></i><i
													class="fa fa-edit text-danger text"></i></a> | <a href="#modal"
												data-href="deleteUser.action?contentId=${item.personId }&menu=1&sortmenu=1&submenu=1"
...
<script type="text/javascript">
	$("a[name='page']").click(
			function() {
				...
				var submenu = ${submenu};
				var url = "teamList.action?" + "size=" + size + "&page="
