	...
	$(document).ready(function() {
		$("div.footer-page-item").click(function() {
			...
			if (page != "...") {
				...
				url = "adminProjectListAdmin.action?";
				url += "page=" + page;
				url += "&size=" + size;
				window.location = url;
			}
		...
		$(".search-img").click(function() {
			...
			url = "adminSearchProjectListByName.action?name=" + name+"&page=0&size=10000";
				...
				<table class="data display datatable" id="example">
					...
					<tbody>
						<c:forEach items="${items}" var="item" varStatus="vs">
							<tr class="odd gradeX">
								<td class="center"><a
									href="adminEditProject.action?projectId=${item.projectId}"
									target="content">${item.projectId}</a></td>
								<td class="center">${item.fullName}</td>
								<td class="center"><a
									href=${item.startPageImage
									} target="blank">${item.startPageImage}</a></td>
