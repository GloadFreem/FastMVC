	...
	$(document).ready(function() {
		$("div.footer-page-item").click(function() {
			...
			if (page != "...") {
				...
				url = "adminSceneListAdmin.action?";
				url += "page=" + page;
				url += "&size=" + size;
				window.location = url;
			}
...
$(".search-img").click(function() {
			...
			url = "adminSearchSceneListByName.action?name=" + name+"&page=0&size=10000";
		...
		<div class="box round first grid">
			<div>
				<a href="adminEditScene.action?sceneId=" target="content"><h2>
						<div>
							<img alt="添加现场" src="images/圆角矩形-3-拷贝-5.png">
						</div>
						<div>添加现场</div>
					</h2></a>
			</div>
				...
				<table class="data display datatable" id="example">
					...
					<tbody>
						<c:forEach items="${items}" var="item" varStatus="vs">
							<tr class="odd gradeX">
								<td class="center"><a
									href="adminEditScene.action?sceneId=${item.sceneId}"
									target="content">${item.sceneId}</a></td>
								<td class="center"><a
									href="adminEditProject.action?projectId=${item.project.projectId}"
									target="content">${item.project.fullName}</a></td>
								<td class="center"><a href=${item.audioPath
									}
									target="content">${item.audioPath}</a></td>
